...
void GameBase::Initialize()
{
	...
	Memory::AddObject(xor ("GameBase::Mode"), xor ("C3 FF 15 ?? ?? ?? ?? 83 3D"), 0x9, 1);

	Memory::AddObject(xor ("GameBase::UpdateTiming"), xor ("55 8B EC 83 E4 F8 57 56 83 EC 18 8B F9 8B 0D"));
	...
	Memory::AddObject(xor ("GameBase::IsFullscreen"), xor ("55 8B EC 57 56 53 8B F1 80 3D ?? ?? ?? ?? 00 75 05 E9 ?? ?? ?? ?? 81 3D"), 0xA, 1);
	//taken from osu!rx, it's not actually gamebase::clientbounds, probably a viewport from glcontrol
	Memory::AddObject(xor ("GameBase::ClientBounds"), xor ("89 45 C8 8B 72 0C 8B 15"), 0x8, 1);

	Memory::AddObject(xor ("GameBase::ClientHash"), xor ("E8 ?? ?? ?? ?? 8B 8D ?? ?? ?? ?? E8 ?? ?? ?? ?? 8D 15"), 0x12, 1);
	Memory::AddObject(xor ("GameBase::UniqueID"), xor ("8D 4D D0 E8 ?? ?? ?? ?? 8B 35"), 0xA, 1);
	Memory::AddObject(xor ("GameBase::UniqueID2"), xor ("EB 10 E8 ?? ?? ?? ?? 89 85 ?? ?? ?? ?? E8 ?? ?? ?? ?? 8B 1D"), 0x14, 1);
	Memory::AddObject(xor ("GameBase::UniqueCheck"), xor ("C6 40 14 00 8B 1D"), 0x6, 1);
