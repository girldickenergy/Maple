    ...
    static __forceinline uintptr_t HdnGetProcAddress(void* hModule, const wchar_t* wAPIName)
    {
        ...
        if (idhDosHeader->e_magic == 0x5A4D)
        {
            ...
            if (inhNtHeader->Signature == 0x4550)
            {
                ...
                {
                    char* szNames =
                        reinterpret_cast<char*>(lpBase + reinterpret_cast<unsigned long*>(lpBase + iedExportDirectory->AddressOfNames)[uiIter]);
                    if (wcscmp(wAPIName, (wchar_t*)GetWC(szNames)) == 0)
                    {
                        ...
                        return reinterpret_cast<uintptr_t>(
                            lpBase + reinterpret_cast<unsigned long*>(lpBase + iedExportDirectory->AddressOfFunctions)[usOrdinal]);
                    }
                }
            }
        }
