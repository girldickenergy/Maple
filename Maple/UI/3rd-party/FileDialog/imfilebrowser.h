...
inline void ImGui::FileBrowser::Display()
{
    ...
    for(auto &sec : pwd_)
    {
        ...
        if(Widgets::SmallButton(u8StrToStr(sec.u8string()).c_str()))
        {
    ...
    if(newDirNameBuf_)
    {
        ...
        if(BeginPopup(openNewDirLabel_.c_str()))
        {
            ...
            if(Widgets::Button("ok") && (*newDirNameBuf_)[0] != '\0')
            {
    ...
    if(!(flags_ & ImGuiFileBrowserFlags_SelectDirectory))
    {
        if(Widgets::Button(" ok ") && !selectedFilenames_.empty())
        {
            ok_ = true;
            CloseCurrentPopup();
        }
    ...
    bool shouldExit =
        Widgets::Button("cancel") || closeFlag_ ||
