...
void Editor::LoadBeatmap(std::string beatmapHash)
{
    ...
    osuPlayfield = OsuPlayfield(drawList, &Editor::bmap, &Time, &Editor::currentFrame, &hitObjects);
    ...
    if (selectedFrames.size() > 0)
    {
        for (int i = 0; i < selectedFrames.size(); i++)
        {
            ...
            if (screenPos2.X != 0 && screenPos2.Y != 0)
                drawList->AddLine(ImVec2(screenPos.X, screenPos.Y), ImVec2(screenPos2.X, screenPos2.Y),
                                  ImGui::ColorConvertFloat4ToU32(ImVec4(COL(51.f), COL(135.f), COL(255.f), 1.f)), 1.f);
            if (Config::ReplayEditor::ShowReplayFrames)
                drawList->AddCircleFilled(ImVec2(screenPos.X, screenPos.Y), 2.f,
                                          ImGui::ColorConvertFloat4ToU32(ImVec4(COL(51.f), COL(135.f), COL(255.f), 1.f)));
        }
    }
}
...
{
    const auto& frames = _replayHandler.GetReplay()->ReplayFrames;

    if (frames.size() <= 1)
        return;

    int currentFrameIndex = currentFrame;
    while (Time < frames[currentFrameIndex].Time && currentFrameIndex > 0)
        currentFrameIndex--;
    while (Time >= frames[currentFrameIndex].Time && currentFrameIndex != frames.size() - 1)
        currentFrameIndex++;
...
void Editor::Render()
{
    ...
    ImGui::Begin(xorstr_("re"), nullptr,
                     ...
                     ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_NoBackground);
    {
        ...
        drawList->AddRectFilled(ImVec2(0, 0), ImVec2(clientBounds.X, clientBounds.Y),
                                ImGui::ColorConvertFloat4ToU32(ImVec4(COL(65.f), COL(65.f), COL(65.f), 1.f)));

        if (IsEditing)
            drawList->AddRectFilled(ImVec2(0, 0), ImVec2(clientBounds.X, clientBounds.Y),
                                    ImGui::ColorConvertFloat4ToU32(ImVec4(COL(255.f), COL(65.f), COL(65.f), 0.2f)));
        ...
        if (Dragging)
            drawList->AddRect(ImVec2(dragArea.first.X, dragArea.first.Y), ImVec2(dragArea.second.X, dragArea.second.Y),
                              ImGui::ColorConvertFloat4ToU32(ImVec4(COL(255.f), COL(255.f), COL(255.f), 0.65f)), 2.f, 0, 2.f);

#pragma region "Draw Topbar"
        float topBarHeight = ((clientBounds.Y * 3.75f) / 100.f) * StyleProvider::Scale;
        drawList->AddRectFilled(ImVec2(0, 0), ImVec2(clientBounds.X, topBarHeight),
...
#pragma endregion
    ...
    if (optionsOpen)
    {
        ...
        const ImVec2 windowSize =
            ImVec2(((clientBounds.X * 35.5f) / 100.f) * 0.8f, panelHeaderHeight + panelContentHeight) + StyleProvider::Padding * 2;
        ...
        ImGui::Begin(xorstr_("re_options"), nullptr, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar);
        {
            ...
            Widgets::BeginPanel(xorstr_("Options"), ImVec2(optionsWidth, panelContentHeight));
            {
                ...
                Widgets::SliderInt(xorstr_("Frame Count"), &Config::ReplayEditor::FrameCount, 5, 50, 1, 10, xorstr_("%d"),
                                   ImGuiSliderFlags_ClampOnInput);
                Widgets::SliderFloat(xorstr_("Timeline Resolution"), &Config::ReplayEditor::TimelineResolution, 0.25f, 2.f, 0.1f, 1, xorstr_("%.1f"),
                                     ImGuiSliderFlags_ClampOnInput);
                if (Widgets::SliderFloat(xorstr_("Playback rate"), &Config::ReplayEditor::PlaybackRate, 0.25f, 2.5f, 0.1f, 1, xorstr_("%.1f"),
                                         ImGuiSliderFlags_ClampOnInput))
                {
                    // AudioEngine::SetCurrentPlaybackRate(Config::ReplayEditor::PlaybackRate * 100.f);
                }
                Widgets::SliderInt(xorstr_("Edit Resync Time"), &Config::ReplayEditor::EditResyncTime, 50, 500, 1, 10, xorstr_("%d"),
                                   ImGuiSliderFlags_ClampOnInput);
            }
            Widgets::EndPanel();
        }
...
void Editor::TimerThread()
{
    ...
    int64_t comparasionTicks = GetTicks();
    while (true)
    {
        if (EditorState == Playing)
        {
            int64_t currentTick = GetTicks();

            const auto tickDifference = static_cast<double_t>(currentTick - comparasionTicks) * (Config::ReplayEditor::PlaybackRate * 1000.f) /
                                        static_cast<double_t>(frequency.QuadPart);

            if (tickDifference >= 1)
            {
                Time++;
                comparasionTicks = GetTicks();
