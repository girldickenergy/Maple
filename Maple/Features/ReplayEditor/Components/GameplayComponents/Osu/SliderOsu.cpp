...
ReplayEditor::SliderOsu::SliderOsu(HitObject* _internalHitObject, int _index, int _time, int _preempt, int* _timer, Vector2 _position,
                                   double _velocity, int _segmentCount, Transformation _transformation, std::vector<int> _sliderScoreTimingPoints,
                                   std::vector<double> _cumulativeLengths, std::vector<std::pair<Vector2, Vector2>> _points,
                                   std::vector<Vector2> _allPoints)
...
void ReplayEditor::SliderOsu::AnalyzeSliderTick(int time, SliderTickOsu* sliderTickOsu)
{
    ...
    if (t != nullptr)
    {
        ...
        if (t->GetEndTime() == t->GetStartTime())
            ...
            position = t->GetStartPosition() + (t->GetEndPosition() - t->GetStartPosition()) *
