...
ReplayEditor::EventTimeline::EventTimeline(int* _timer, ImDrawList* _drawList, Replay* _replay, Vector2 _clientBounds,
                                           std::vector<HitObject>* _hitObjects, int _od, int _cs)
{
    timer = _timer;
    drawList = _drawList;
    replay = _replay;
...
void ReplayEditor::EventTimeline::ParseEvents(std::vector<ReplayEditor::OsuDrawable*> drawables)
{
    ...
    for (auto current = drawables.begin(); current != drawables.end(); ++current)
    {
        ...
        if (drawablePointer->GetDrawableType() != DrawableType::Drawable_HitObjectOsu &&
            drawablePointer->GetDrawableType() != DrawableType::Drawable_HitObjectSliderOsu)
            continue;
...
void ReplayEditor::EventTimeline::Draw()
{
    ...
    // Upper border
    drawList->AddRectFilled(ImVec2(0, clientBounds.Y - clickTimelineHeight - eventTimelineHeight - 2),
                            ImVec2(clientBounds.X, clientBounds.Y - clickTimelineHeight - eventTimelineHeight),
                            ImGui::ColorConvertFloat4ToU32(ImVec4(COL(51.f), COL(51.f), COL(51.f), 1.f)));

    drawList->AddRectFilled(ImVec2(0, clientBounds.Y - clickTimelineHeight - eventTimelineHeight),
                            ImVec2(clientBounds.X, clientBounds.Y - clickTimelineHeight),
                            ImGui::ColorConvertFloat4ToU32(ImVec4(COL(71.f), COL(71.f), COL(71.f), 1.f)));

    // Under border
    drawList->AddRectFilled(ImVec2(0, clientBounds.Y - clickTimelineHeight - 2), ImVec2(clientBounds.X, clientBounds.Y - clickTimelineHeight),
                            ImGui::ColorConvertFloat4ToU32(ImVec4(COL(51.f), COL(51.f), COL(51.f), 1.f)));

    EventTimelineLocation = std::make_pair(Vector2(0, clientBounds.Y - clickTimelineHeight - eventTimelineHeight),
                                           Vector2(clientBounds.X, clientBounds.Y - clickTimelineHeight));
    ...
    for (auto& e : events)
    {
        if (e.eventType != EventType::ThreeHundred)
        {
            ...
            drawList->AddRectFilled(ImVec2(x - .5f, clientBounds.Y - clickTimelineHeight - eventTimelineHeight),
                                    ImVec2(x + .5f, clientBounds.Y - clickTimelineHeight - 2), col);
        }
    }

    int timeX = EventTimeline::TimeToX(*timer);
    drawList->AddRect(ImVec2(timeX - 2.f, clientBounds.Y - clickTimelineHeight - eventTimelineHeight),
                      ImVec2(timeX + 2.f, clientBounds.Y - clickTimelineHeight - 2),
                      ImGui::ColorConvertFloat4ToU32(ImVec4(COL(255.f), COL(255.f), COL(255.f), 0.5f)));
}
